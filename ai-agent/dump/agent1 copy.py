...
class Agent1(Agent):
    ...
    async def maybe_speak_service_info(self, history):
        """
        If a new service is detected, speak RAG info about it.
        """
        service = self.agent2.service
        if service and service != self._last_service_seen:
            self._last_service_seen = service
            await self.safe_say("Wait for a sec, I am gathering info on service.")
            # Use the last user utterance or service name for RAG
            try:
                # Use the last user message if available, else service name
                if history and isinstance(history[-1], ChatMessage):
                    query_text = history[-1].content[0] if isinstance(history[-1].content, list) else history[-1].content
                else:
                    query_text = service
                # Call the RAG API
                payload = {
                ...
                if resp.status_code == 200:
                    ...
                    if results:
                        spoken = results[0].get("text", str(results[0]))
                        await self.safe_say(spoken)
                    else:
                        await self.safe_say("Sorry, I could not find information on that service.")
                else:
                    await self.safe_say("Sorry, I could not retrieve service info right now.")
            except Exception as e:
                logger.error(f"[Agent1] Failed to get/speak RAG info: {e}")
